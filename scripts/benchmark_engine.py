...
def display_ml_validation_metrics():
    ...
    try:
        ...
        print(f" Anti-Leakage Actif   : {'✅ OUI (shift causal + médiane train)' if data.get('anti_leakage_applied') else 'NON'}")
        print(f" Médiane Imputée (Tr) : {data.get('train_median_imputed')} min")
        print(f" Échantillons         : {data.get('n_samples_total')} (Train: {data.get('n_train')}, Val: {data.get('n_val')}, Test: {data.get('n_test')})")
        print("-" * 80)
